...
public class ETCJoystick : ETCBase,IPointerEnterHandler,IDragHandler, IBeginDragHandler, IPointerDownHandler, IPointerUpHandler {
	...
	public void OnDrag(PointerEventData eventData){
		...
		//Vector2 delta = cachedRectTransform.anchoredPosition - center;
		//thumbPosition = (delta + eventData.position - eventData.pressPosition) / cachedRootCanvas.rectTransform().localScale.x;

		//thumbPosition =  eventData.position/ cachedRootCanvas.rectTransform().localScale.x - eventData.pressPosition/ cachedRootCanvas.rectTransform().localScale.x;
		...
		thumb.anchoredPosition =  thumbPosition; //+ eventData.delta;
	...
	private void UpdateJoystick(){
		...
		axisX.UpdateAxis( tmpAxis.x,isOnDrag, ETCBase.ControlType.Joystick);
		axisY.UpdateAxis( tmpAxis.y,isOnDrag, ETCBase.ControlType.Joystick);
