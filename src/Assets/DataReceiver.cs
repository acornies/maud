	...
	public void ProcessMessage(Stream stream)
	{
		...
		if (Enum.IsDefined(typeof(EditorMessage), (EditorMessage)msg))
		{
			switch ((EditorMessage)msg)
			{
				...
				case EditorMessage.LocationServiceStart: HandleLocationServiceStart(reader); break;
				case EditorMessage.LocationServiceStop:  HandleLocationServiceStop(reader); break;
				case EditorMessage.CompassSetEnabled:    HandleCompassSetEnabled(reader); break;
			}
		}
		else
		{
